...
namespace TailwindCSSIntellisense.Completions.Controllers
{
    ...
    internal sealed class CssCommandFilter : IOleCommandTarget
    {
                ...
                if (pguidCmdGroup == VSConstants.VSStd2K)
                {
                    switch ((VSConstants.VSStd2KCmdID)nCmdID)
                    {
                        case VSConstants.VSStd2KCmdID.TYPECHAR:
                            ...
                            if (isInApply && CharsAfterSignificantPoint(classText) <= 2 || character == ' ' || character == ':')
                            {
                        ...
                        case VSConstants.VSStd2KCmdID.BACKSPACE:
                            ...
                            if (isInApply && CharsAfterSignificantPoint(classText) <= 2)
                            {
                                _currentSession?.Dismiss();
                                StartSession(true);
                            }
        ...
        private string GetClassText()
        {
            ...
            return text.Substring(lastIndexOfAt + "@apply".Length).Trim();
        }
    }
