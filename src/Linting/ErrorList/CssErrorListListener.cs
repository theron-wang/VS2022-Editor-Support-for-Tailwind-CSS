...
public class CssErrorListListener : ITextViewCreationListener
{
    [Import]
    ...
    {
        if (_linterUtilities.LinterEnabled())
        {
            if (_contexts.ContainsKey(view.TextBuffer) == false)
            {
                ...
                var validator = CssValidator.Create(view.TextBuffer, _linterUtilities, _completionUtilities);
